...
export function createServer() {
  ...
  if (!owner || !repo) {
    throw new Error('GitHub repository owner and name must be configured in environment variables.');
  }

  app.get('/articles', async (req, res) => {
    try {
      const contents = await githubService.getContent(owner, repo, articlesDir);
      const articles = contents.filter((content) => content.type === 'file' && content.name.endsWith('.md'));
      res.json(articles);
    } catch (error) {
      console.error(error);
  ...
  app.get('/articles/:name', async (req, res) => {
    try {
      ...
      const content = await githubService.getFileContent(owner, repo, articlePath);
      res.send(content);
    } catch (error) {
      console.error(error);
