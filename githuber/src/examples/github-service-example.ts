...
async function demonstrateGitHubService() {
  ...
  try {
    ...
    console.log(`Reset time: ${new Date(rateLimit.rate.reset * 1000).toISOString()}\n`);
    ...
    try {
      const readmeContent = await github.getFileContent('microsoft', 'vscode', 'README.md');
      console.log(`README.md content length: ${readmeContent.length} characters`);
      console.log(`First 200 characters: ${readmeContent.substring(0, 200)}...\n`);
    } catch (error) {
    ...
    const commits = await github.getCommits('microsoft', 'vscode', { perPage: 5 });
    ...
    console.log(`Found ${searchResults.totalCount} repositories matching "typescript react":`);
...
async function getRepositoryArticles(owner: string, repo: string, path: string = '') {
  const github = new GitHubService();
  
  try {
    // Get all markdown files from a specific path
    const contents = await github.getContent(owner, repo, path);
    const articles = contents.filter(item => 
      item.type === 'file' && 
...
export {
  demonstrateGitHubService,
  getRepositoryArticles
};
