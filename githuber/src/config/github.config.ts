...
export const validateGitHubConfig = (): void => {
  if (!githubConfig.token) {
    console.warn('⚠️  GITHUB_TOKEN environment variable is not set. API requests will be limited to unauthenticated rate limits.');
  }
  
  if (githubConfig.rateLimit < 1 || githubConfig.rateLimit > 100) {
    console.warn('⚠️  GITHUB_RATE_LIMIT should be between 1 and 100 requests per minute. Using default value of 60.');
